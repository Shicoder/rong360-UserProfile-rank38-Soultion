...
def display_precision_recall(y_, oof_preds_, folds_idx_):
    ...
    plt.plot(precision, recall, color='b', label='Avg ROC (APC = %0.4f $\pm$ %0.4f)' % (score, np.std(scores)), lw=2,
             alpha=.8)
...
def application_train_test(num_rows=None, nan_as_category=True):
    ...
    df['EXT_SOURCES_NULL_NUM'] = np.sum(df[['EXT_SOURCE_1', 'EXT_SOURCE_2', 'EXT_SOURCE_3']].isnull(),
                                        axis=1)  # 外部数据的均值
...
def bureau_and_balance(feature):
    ...
    data['CREDICT_SUM_LEFT'] = data['AMT_CREDIT_SUM'] - data['AMT_CREDIT_SUM_DEBT'] - data[
        'AMT_CREDIT_SUM_LIMIT']  # 信用余额
...
def credit_card_balance(feature):
    ...
    cc['AMT_DRAWINGS_POS_CURRENT_PER'] = cc['AMT_DRAWINGS_POS_CURRENT'] / cc[
        'CNT_DRAWINGS_ATM_CURRENT']  # 购买商品取款额 / 总取款额
...
sub_file = '../sub/submission_5x-average-LGB-run-01-v1_loss_1fold' + score + '_' + str(
    now.strftime('%Y-%m-%d-%H-%M')) + "_seed_" + str(seed) + '.csv'
